...
const CommissionSetup = () => {
  ...
  const Formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      BackendAxios.post("/api/admin/create-package", values)
        .then(() => {
          Toast({
            status: "success",
  ...
  function fetchAllPackages(pageLink) {
    BackendAxios.get(pageLink || "/api/admin/packages?page=1")
      .then((res) => {
        setPagination({
  ...
  function fetchAllCommission(selectedPackageId, serviceName, emptyRow) {
    BackendAxios.get(
      `api/admin/commissions/${serviceName}/${selectedPackageId}`
    )
      .then((res) => {
  ...
  function onCellValueChange(params) {
    if (selectedService != "aeps-mini-statement" && selectedService != "bbps") {
  ...
  async function deleteCommission(keyword, id) {
    await BackendAxios.post(`/api/admin/commissions/delete/${keyword}/${id}`)
      .then((res) => {
        Toast({
          status: "success",
  ...
  useEffect(() => {
    if (selectedService == "bbps") {
      BackendAxios.get("/api/admin/operators")
        .then((res) => {
          ...
          onClick={() => {
            if (!params.data.id) {
              params.api.applyTransaction({
                remove: params.api.getSelectedRows(),
              });
              return;
            }
  ...
  useEffect(() => {
    ...
    BackendAxios.get("/api/admin/package-count")
      .then((res) => {})
      .catch((err) => {
        if (err?.response?.status == 401) {
          Cookies.remove("verified");
  ...
  const verifyBeneficiary = (queriedUserId) => {
    ...
    BackendAxios.post(
      `/api/admin/user/info/${VerificationFormik.values.beneficiaryId}`
    )
      .then((res) => {
  ...
  return (
    <>
      <Layout pageTitle={"Define Commission Rate"}>
        ...
        <Stack spacing={6} my={6} direction={["column", "row"]}>
          <FormControl w={"xs"}>
        ...
        <Box mt={"28"}>
          ...
          <HStack mt={6} mb={2}>
            ...
            />
            <Button onClick={searchPackage} colorScheme={"twitter"}>
              Search
            </Button>
          </HStack>
                    ...
                    <Tr key={key}>
                      ...
                      <Td>
                        <Input
                          ...
                          onChange={(e) =>
                            setFocussedPackageTitle(e.target.value)
                          }
                          p={0}
                          border={0}
                          variant={"unstyled"}
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <HStack justifyContent={"flex-end"} spacing={4}>
              <Button variant={"ghost"} onClick={() => setModalStatus(false)}>
                Cancel
              </Button>
            </HStack>
          </ModalFooter>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>This action can not be undone!</ModalHeader>
          <ModalBody>
            <Text>
              By deleting this package, you understand that it can't be
              recovered back.
            </Text>
            <Text fontWeight={"bold"}>Enter your MPIN to confirm.</Text>
          ...
          <ModalFooter>
            <HStack justifyContent={"flex-end"}>
              <Button colorScheme="red" onClick={deletePackage}>
                Delete Now
              </Button>
            </HStack>
          </ModalFooter>
      ...
      >
        ...
        <ModalContent>
          <ModalHeader>
            Assign {isAssignModalOpen.name} Package To Any User
          </ModalHeader>
          <ModalBody>
          ...
          <ModalFooter>
            <HStack justifyContent={"flex-end"}>
              <Button colorScheme="twitter" onClick={() => assignPackage()}>
                Confirm
              </Button>
            </HStack>
          </ModalFooter>
